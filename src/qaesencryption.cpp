...
QAESEncryption::QAESEncryption()
    : m_nb(4), m_blocklen(16), m_level(AES_128), m_mode(ECB), m_padding(ZERO)
{
    m_state = NULL;
...
QString QAESEncryption::startDecoding(QString text)
{
    qDebug() << "Scanned: " << text;
    // ^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$
    // https://stackoverflow.com/questions/475074/regex-to-parse-or-validate-base64-data
    QRegExp reBase64("^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$");
    if(reBase64.exactMatch(text)) {
        // It's encoded data
        // .append("\0\0\0\0")
//        qDebug() << "Is base64";
        QByteArray encoded_base64 = QByteArray::fromStdString(text.toStdString());
//        qDebug() << "Encoded base 64: " << encoded_base64;
        QByteArray decoded_base64 = QByteArray::fromBase64(encoded_base64);
//        qDebug() << "Decoded base 64: " << decoded_base64;
    //    encoded = this->encode(myText.toUtf8(), "aurora.scan");
        QString decoded = this->decode(decoded_base64, "scan.aurora.scan");
//        qDebug() << "Decoded: " << decoded;
        if (decoded.contains("Паспорт") && decoded.contains("Заявка #")) {
            return decoded + QString("Да\r\n");
        }
        return QString("Неверный формат");
//        if(this->is_correct) {
//            QStringList list = decoded.split("\r\n");
//            this->_name = list[0];
//            this->_birthday = list[1];
//            this->_number = list[2].right(10);
//            this->_id = list[3].right(11);
//        }
//        return;
    }
    else {
        // It's unencoded or incorrect data
//        qDebug() << "Is not base64";
//        qDebug() << "Is valid: " << (text.contains("Паспорт") && text.contains("Заявка #"));
        if (text.contains("Паспорт") && text.contains("Заявка #")) {
            if (!text.endsWith("\r\n"))
                text = text + QString("\r\n");
            return text + QString("Нет\r\n");
        }
        return QString("Неверный формат");
    }
//    qDebug() << "Is base64: " << re.exactMatch(text);
//    QByteArray enc = text.toUtf8().append("\0\0\0\0").toBase64();
//    qDebug() << "Encoded";
//    QString dec = this->decode(enc, "aurora.scan");
//    qDebug() << "Decoded: " << dec;
////    qDebug() << enc;
//    QString myText = QString("Виктор Петрович Ф.\r\n18.03.1978\r\nЗаявка #1234567890\r\nПаспорт **** ***483\r\n");
//    qDebug() << "Original: " << myText;
//    QByteArray encoded = this->encode(myText.toUtf8(), "scan.aurora.scan");
//    qDebug() << "Utf8: " << myText.toUtf8();
//    QByteArray encoded_base64 = encoded.toBase64();
//    qDebug() << "Encoded base 64: " << encoded_base64;
//    QByteArray decoded_base64 = QByteArray::fromBase64(encoded_base64);
//    qDebug() << "Decoded base 64: " << decoded_base64;
////    encoded = this->encode(myText.toUtf8(), "aurora.scan");
